...
public class LtiCoursesOkapiClientFactory {
  private static final Logger logger = LogManager.getLogger(LtiCoursesOkapiClientFactory.class);

  private LtiCoursesOkapiClientFactory() {
  }
  ...
  public static OkapiClientFactory createInstance(Vertx vertx, JsonObject config) {
    ...
    if (isSslEnabled) {
      ...
      return new OkapiClientFactory(vertx, okapiUrl, reqTimeoutMs, keystoreType, keystoreProvider, keystorePath, keystorePassword, keyAlias, keyAliasPassword);
    } else {
      return new OkapiClientFactory(vertx, okapiUrl,  reqTimeoutMs);
    }
  }
}
